...
q3 <- quantile(maleCoef$male_coef, 0.75)
q1 <- quantile(maleCoef$male_coef, 0.75)
IQR <- IQR(maleCoef$male_coef)

maleCoef <- maleCoef %>% filter(male_coef <= q3 + 1.5*IQR)
maleCoef <- maleCoef %>% filter(male_coef > q1 - 1.5*IQR)

reg <- lm(maleCoef$male~maleCoef$suspects)

plot(maleCoef$male, reg$residuals)
