...
germanCasesSetQuartile <- quantile(germanCasesSet$suspects/germanCasesSet$solved_cases, 0.75)
germanCasesSetQuartile2 <- quantile(germanCasesSet$suspects/germanCasesSet$solved_cases, 0.75)
IQR <- IQR(germanCasesSet$suspects/germanCasesSet$solved_cases)

germanCasesSet <- germanCasesSet %>% filter(suspects/solved_cases <= germanCasesSetQuartile + 1.5*IQR)
germanCasesSet <- germanCasesSet %>% filter(suspects/solved_cases > germanCasesSetQuartile2 - 1.5*IQR)
...
summary(reg)
```
The outliers that we determined before are clearly visible due to the high deviation in the minimum and maximum values of the residuals compared with
the quartiles. What we can see is, When solved cases are zero, the amount of suspects is -170,71 and with each increase of solved cases by one, the amount of suspects rises by 1,13. Since we can see already that the standard error of solved cases is about 127 cases, the deviation is way too high, at least for the intercept of our model. This impression is confirmed by the probability value, that determines whether suspects do not influence the amount of solved cases, also known as null hypthesis: For solved cases at the intercept, it is 0,18 and therefore, higher than the cut off score of 0,05. The evidence is way too low to asssume that suspects really affect the amount of cases, but only at this point (the intercept).This is confirmed by the second probability value: This one is way below 0,05, the null hypothesis is not confirmed, which means: suspects are statistically significant for a change of solved cases.  Probably the variation of suspects and solved cases is just too high, the degree of freedom shows a high population.  <br/>
