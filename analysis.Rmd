...
The "SumKeys" data set was created by us, therefore every optimization for importing were made while creating the set, with the use of an online tool scraping the keys and their type from the "Straftatenkatalog" pdf-file. <br/> 
Finally we also import the data set for the inhabitants, since this data set is not part of the main analysis, we decided to edit it in
...
plot(germanCasesSet$suspects, reg$residuals)

summary(reg)
```
The outliers that we determined before are clearly visible due to the high deviation in the minimum and maximum values of the residuals compared with the quartiles. Furthermore what we can see is that the models standard errors are low, but the residual standard error is high. This is due to the high outliers mentioned already. That tells us we have to get rid of these outliers in order to improve the models fit:
```{r}
library(DescTools)

#find Q1
Q1 <- quantile(germanCasesSet$solved_cases, .05)
#find Q3 
Q3  <- quantile(germanCasesSet$solved_cases, .95)
...
summary(reg)
```
What we can see is, When solved cases are zero, the amount of suspects is -18,64 and with each increase of solved cases by one, the amount of suspects rises by ~ 1,04 Since we can see already that the standard error of solved cases is about 26,68 cases, the deviation at the intercept is high. This impression is confirmed by the probability value, that determines whether suspects do not influence the amount of solved cases, also known as null hypthesis: For solved cases at the intercept, it is 0,432 and therefore, higher than the cut off score of 0,05. The evidence is way too low to asssume that suspects really affect the amount of cases, but only at this point (the intercept). <br/>
The second probability value on the other hand confirms: It is way below 0,05, the null hypothesis is not likely to be true, which means: Suspects are statistically significant for a change of solved cases.  Probably the variation of suspects and solved cases is just too high, the degree of freedom shows a high population.  <br/>
Also, the residual standard error follows the impression of the SSE: It is very high, although we have a high amount of freedom degrees. (710 rows - 2 variables in the model = 708 DF). Looking at the R squared tells us here that, although the RSE and the SSE seem to be very high, the model determines a good fit: 0,9534.  Therefore, 95 percent of the correlation is explained by the model. What we have learned from the outliers analysis as well was that there can be crimes that can only be commmited my more than one suspects. This also explains the deviation a bit. In conclusion, the model tells us a good fit, but its needs to be taken into account that the amount of DF is very high, therefore the variation can be and is influenced by other variables.  <br/>
...
hist(reg$residuals)
```
It confirms the sum and our impression that the suspects are a factor for the change of solved cases.  <br/>
After reducing the samplesize by filtering out outliers, we can see an almost equal spread of positive and negative residuals. 
A more detailed analysis is provided by getting the plot of reg:  <br/>
```{r}
par(mfrow=c(2,2))
plot(reg)
par(mfrow=c(1,1))
```
##### Residuals vs Fitted 
As we can see, the red line follows the dotted line because of the equal spread of positive and negative residuals. Also there are a few observations that highly influence the model, in the following rows of the germanCasesSet: 570,289 and 274. It confirms the impression of the histogram generated before: The residuals are equally spreaded.

##### Normal Q Q 
We can see skewed residuals in the upper and lower quantiles. This tells us what we already thought. The spread inside the population is not equal, especially at the end of the line. This is due to a less observations of solved cases in a range greater than 10.000.

##### Residuals vs Leverage 
There are a few influential cases here, but only two of them are above the dotted line: 289 and 274. Since there are a lot of values at the start of the dotted line, we have to be careful interpreting predicted values.

##### Scale location
This plot also shows the equal spread of residuals. As we can see, the red line rises only a bit, this shows that the assumption of homoscedasticity is almost given. The residuals seem also not to be randomly scattered.
...
We can try to predict the solved cases when we know x. (The amount of suspects) to compare the calculated value with the actual of 1334 cases by 1562 suspects.:  <br/>
Y = 1,047679*1562-18,641053   <br/>
Y = 1.617,833545  <br/>

The model predicts an amount of ~ 1617,83 solved cases when we have 1562 suspects.  <br/>
The same result is displayed by applying predict():   <br/>
```{r}
p <- as.data.frame(1562)
colnames(p) <- "suspects"
...
plot the accuracy in a boxplot ofterwards.
