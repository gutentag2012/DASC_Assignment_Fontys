...
We can see that, when suspects are zero, the amount of solved_cases is about 11 and with each increase of suspects by one, the amount of solved cases 
increases by 0,92. With about 10 the standard error at the intercept is extremely high, and the null hypothesis has a high percentage to be true. 
...
regression model, but has to be kept in mind. <br/>
...
When looking at the RSS of both models it, the RSS of the full model, the one with the multiple linear regression, is much lower and therefore more 
accurate than the other one. Also the null hypothesis can be rejected when looking at the p-value, therefore we can proceed with the full model. The 
next step would be to assure that there is no collinearity between the different variables of the multiple linear regression, but since we are working
with a categorical variable we cant check for collinearity and only look at the summary for the model.

```{r Analyse_Regression_Values}
summary(reg)
```

The summary for the different variables affecting the linear regression shows, that the null hypothesis is not valid for most of the variables, only 
the fraud category has a almost 8 percent chance for the null hypothesis to be accepted. We could not find a reason for this, but what we noticed is,
that the estimate for the theft category shows a larger impact than the other variables and is also negative, which could be explained by their optimal
curve having a lower slope in the plot before. Also the estimated impact of the categories is in general lower compared
to the estimate of the relevance which tells us, that the categories are not the main influence in this model. There is one category missing in the
summary for the regression model, we can only assume that this is an error from the program and that there is no significant difference to the other 
categories, so we keep analyzing. <br/>
...
To be sure about the validity of the and power of this model, we take a look at the residuals for the regression model.
...
ggplot(p %>% filter(offset < 100, offset > -100), aes(category, offset, fill = category)) +
  geom_boxplot() +
  scale_x_discrete(guide = guide_axis(n.dodge = 3)) +
  labs(
...
Predicting values with the linear model can lead to large differences in the observations, that is why we filtered the larger outliers for the boxplot
to get a better view on the important values.

```{r Amount_Filtered_Values}
oldRows <- nrow(p)
filteredRows <- nrow(p %>% filter(offset < 100, offset > -100))
(oldRows - filteredRows) / oldRows
```

So in the boxplot view we filtered 14% of the predicted values with a large offset, but after that the plots displayed gather around 0 and have a very
small offset. So in general the accuracy of the regression model is high, and the three categories mentioned in the first chapter with the most consistent
and high solvage rate, have the most accurate predicted values. Most of the highest outliers are part of the theft category which makes sense, since 
their solvage rate also differs a lot from the others. <br/>
Also the two categories with the highest p-value in the regression model also have the highest median in the boxplots, so the values predicted for these
categories is not as close as the one from the other categories. The one category that was missing in the regression summary has a high inner quartile 
range, but also the median around 0. This boxplot proves the assumption that we made, that the regression values for this category are not far from
...
The general accuracy of the predictions is around 79% (100% - 21%  offset), and interestingly the same value as the r squared of the regression model. 
In general when looking at the mean and median, the regression model has a high accuracy and is therefore accepted. <br/>
<br/>
