...
ggplot(solvedCasesFiltered, aes(solved_cases, suspects, color = category)) +
  geom_point() +
  geom_smooth(method = "lm", fullrange = F, show.legend = F) +
  geom_abline() +
...
```{r Correlation_Check}
summary(germanCaptureSet$solved_cases)
nrow(germanCaptureSet %>% filter(solved_cases > 10000))
germanCasesSet <- germanCaptureSet %>% filter(detected_cases > 0, solved_cases > 0, solved_cases <= 10000)
summary(germanCasesSet$solved_cases)
