...
```{r}
ggplot(filtered, aes(solved_cases/detected_cases, suspects, color = category)) +
  geom_point() +
  geom_smooth(method = "lm", fullrange = T, show.legend = F)
```
...
province_filtered %>% merge(province_ids, "province_id") %>% arrange(province_id)
...
When looking on this map, it is clearly visible, that there are
...
ggplot(map_data, aes(long, lat, group = group, fill = crime_rate), color = "black") +
  geom_polygon() +
...
```{r}
f <- provinceCaptureSet %>% 
  filter(detected_cases > 0 & detected_cases >= solved_cases) %>% 
  mutate(solvage_rate = solved_cases / detected_cases) %>% 
  mutate(crime_rate = detected_cases / inhabitants * 10000)

f %>% group_by(province) %>% 
  summarise(avg = mean(crime_rate)) %>% 
  arrange(desc(avg))
```

```{r}
ggplot(f, aes(province, solvage_rate, fill = province)) +
  geom_boxplot() +
  scale_x_discrete(guide = guide_axis(n.dodge=2))
```

```{r}
f %>% filter(is.na(category))
```
...
```{r}
barplot(sort(filtered$solvage_rate))
abline(coef = c(0.69952,0), col = "#cc0000")
abline(coef = c(0.80491,0), col = "#0000cc")
abline(coef = c(0.47919,0), col = "#00cc00")
abline(coef = c(0.94299,0), col = "#00cc00")
hist(filtered$solvage_rate)
boxplot(filtered$solvage_rate)
```
