...
library(reshape)
meltedGerman <- melt(filter(germanCaptureSet, detected_cases > 0), id = c("key", "category", "province", "inhabitants", "detected_cases", "solved_cases", "not_executed_cases", "suspects", "male", "female", "non_german", "gun_threatened", "gun_shot", "province_id"))
detach("package:reshape", unload=TRUE)
meltedGerman$value <- as.integer(meltedGerman$value)
meltedGerman %>% arrange(key)
```


Shorten set and do stats for different distributions 
...
ggplot(filter(meltedGerman, value > 0)) +
  geom_boxplot(aes(relevance, variable)) +
  geom_boxplot(aes(rel_solvage, variable), color = "red", fill = "transparent")

ggplot(filter(meltedGerman, value > 0)) +
  geom_boxplot(aes(shift, variable))

ggplot(filter(meltedGerman, value > 0), aes(relevance, rel_solvage, color = variable)) +
  geom_point() +
  facet_wrap(~variable)
```

```{r}
reg <- lm(meltedGerman$rel_solvage~meltedGerman$relevance)

cor(meltedGerman$relevance,meltedGerman$rel_solvage)
...
```
```{r}
plot(meltedGerman$relevance, meltedGerman$rel_solvage)
