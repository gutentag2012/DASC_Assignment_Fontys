...
```
As can be seen, most of the values are below 100.000 (amount of cases). Thats why we filter out values above that value now. 
```{r}
solvedCasesFiltered <-filter(germanCaptureSet, germanCaptureSet$solved_cases<=100000)
```
Now lets run the plot again: 
```{r}
ggplot(data = solvedCasesFiltered, mapping = aes(x = solved_cases,y = suspects )) +
  geom_point() +
  geom_abline() +
  labs(x = "Solved cases", y = "Suspects")
```
...
Lets take a detailed look into the filtered plot now, by differenciating between the categories: 


```{r}
summary(solvedCasesFiltered)

```



The correlation in the graph above is  also proven by the manual cor command: 
```{r}
cor(germanCaptureSet$solved_cases, germanCaptureSet$suspects)
```
We have a correlation of 98%. 
What we already know from the last chapter, there is also a correlation between detected and solved cases. 
Thats why we are going to use the solved cases now. The  Rows where detected cases have an higher amount than solved cases can be considered as an error. 

Because of the high correlation we can try to predict values with a linear model:
...
```{r}
(reg <- lm(solvedCasesFiltered$solved_cases~solvedCasesFiltered$suspects))
```
What do we get? 
```{r}
summary(reg)
```
What tells us this?
If the amount of solved cases is zero, the amount  of suspects is -0,28.  With every increase of solved cases by one, the amount of suspects increases by 1,04.  
...
```{r}
sum(reg$residuals)
plot(solvedCasesFiltered$solved_cases,reg$residuals, ylab = "Residuen", xlab = "Solved cases")
```
Since this result is almost 0 we can consider the model as accurate. 
...
```
### Residuals vs Fitted 
As we can see, we have a almost perfect spread of residuals above and below the red dotted line, even though there are a few outliers (240,252,368). 
But since there are so many values we have to countercheck the spread with a histogram:
```{r}
hist(reg$residuals)
```
As we can see here, the spread is unfortunately not equal: residuals where are more often negative which shows us that the predictions tend to be higher than the observed solved cases. However, the frequency of residuals is still high. 

### Normal Q Q 
Besides the quantile range from -3 to -2 and 2 to 3 we can see a normal distribution of residuals. 

### Residuals vs Leverage 
There are a few influential cases here, but only of them is positive: 309, above the red dotted 1,0 line. The reason the regression line is still going down is due to the fact that a lot of residuals is below x = 0, and even two of them  have a high influence: 123 and 119.     
