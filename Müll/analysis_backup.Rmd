...
```
The correlation leads to the question: Is the amount of solved cases affected by the amount of suspects? How much of this correlation could be explained by a linear model? This is what we want to look into now: 
```{r}
...
```{r}
q3 <- quantile(germanCasesSet$suspects,.75)
val <- filter(germanCasesSet, suspects <= q3) 
(max <- max(val$suspects))
```
Therefore, 1365 x 1,137880 =
### Solved Cases vs Suspects
Now that we analyzed the aspects of the solvage rate in Germany, We wanted to have a look at the suspects involved in solving a case. How many suspects
do you need to close a case? Are more suspects better for solving the case? <br/>
One note upfront for this, if a group of people is committing a crime together, every one of them will get their own law suite, which means that every 
...
```
These are the values of our set. 
Now lets compare the number of suspects calculated by the model using Y (solved cases) = mx + b: 
1.267 = 1.137880 * x -170,711755   | +170,711755
1.437,711755 = 1,137880 * x        | /1,137880
1.263,500329 = x (amount of suspects)

This is the predicted amount of suspects.  The actual amount of suspects is 1675. 
But we can also try to predict the solved cases when we know x. (The amount of suspects) to compare the calculated value with the actual of 1267 cases by 1675 suspects.: 
Y = 1.137880*1675 - 170,711755    
Y = 1735,237245

The model predicts an amount of ~ 1735,23 solved cases when we have 1675 suspects. 
The same result is displayed by applying predict(): 
```{r}
...
(1 - (max/predicted))*100
```
The deviation about 26 percent. This is a high deviation and it confirmed what we expected earlier: Solved cases are also influenced by other factors, but the amount of suspects definetely has a high influence. In conclusion: The model predictions for a less amount of solved cases are not accurate, as well as when it comes to solved cases at the end because of outliers. The high deviation in the example we are using here is also explained because of the intercept we had to use in the formula. Since the intercept itseld has a high SSE and a high P value, it explains the deviation mentioned above. To check the influence of outliers in the dataframe we can filter by the category with the less amount of outliers which was "Straftaten gegen das Leben" (Crimes against life.)
...
### Model
Since we know already the amount of solved cases is already influenced by the amount of suspects, we want to find out the impact of the population size at the crime scene. The dataframe contains several columns of this, lets see: 
...
```{r}
(reg <- lm(provinceSample$solved_cases~provinceSample$))
